...
def print_results(name, pillow_result, imgrs_result):
    ...
    print(f"  {name}")
    print(f"{'='*60}")
    print(f"  Pillow:  {format_time(pillow_result['mean'])} ± {format_time(pillow_result['stdev'])}")
    print(f"  imgrs:   {format_time(imgrs_result['mean'])} ± {format_time(imgrs_result['stdev'])}")
...
def main():
    ...
    print(f"  Iterations: 10 (with 2 warmup runs)")
    print("="*60)
    ...
    print_results(
        "Load Image",
        benchmark(pillow_load),
        benchmark(imgrs_load)
    )
    ...
    print_results(
        "Resize (400x300)",
        benchmark(pillow_resize),
        benchmark(imgrs_resize)
    )
    ...
    print_results(
        ...
        benchmark(pillow_blur),
        benchmark(imgrs_blur)
    )
    ...
    print_results(
        "Sharpen",
        benchmark(pillow_sharpen),
        benchmark(imgrs_sharpen)
    )
    ...
    print_results(
        "Adjust Brightness",
        benchmark(pillow_brightness),
        benchmark(imgrs_brightness)
    )
    ...
    print_results(
        "Adjust Contrast",
        benchmark(pillow_contrast),
        benchmark(imgrs_contrast)
    )
    ...
    print_results(
        "Rotate 45°",
        benchmark(pillow_rotate),
        benchmark(imgrs_rotate)
    )
    ...
    print_results(
        ...
        benchmark(pillow_grayscale),
        benchmark(imgrs_grayscale)
    )
    ...
    print_results(
        "Crop (200x200)",
        benchmark(pillow_crop),
        benchmark(imgrs_crop)
    )
    ...
    print_results(
        "To Array/Bytes",
        benchmark(pillow_to_array),
        benchmark(imgrs_to_array)
    )
