...
try:
    import imgrs
    from imgrs import Image, BlendMode, ColorMixin, ColorFormat, MaskType
    ...
    print(f"✓ Set global alpha to 0.5")
    ...
    print(f"✓ Extracted red color")
    
    # Color quantization
    img_quantized = img.color_quantize(levels=8)
    print(f"✓ Applied color quantization (8 levels)")
    
    # Color shift
    img_shifted = img.color_shift(0.2)
    print(f"✓ Applied color shift")
    
    # Selective desaturation
    img_selective = img.selective_desaturate((255, 0, 0), tolerance=50, desaturate_factor=0.7)
    print(f"✓ Applied selective desaturation")
    ...
    print(f"✓ Applied horizontal gradient overlay")
    
    # Create stripe pattern
    stripe_pattern = img.create_stripe_pattern((0, 0, 255, 128), width=10, spacing=5, angle=45.0)
    print(f"✓ Created diagonal stripe pattern")
    
    # Create checker pattern
    checker_pattern = img.create_checker_pattern((255, 0, 0, 100), (0, 0, 255, 100), size=8)
    print(f"✓ Created checkerboard pattern")
    ...
    print(f"✓ Split image into RGB and alpha channels")
    
    # Merge alpha
    img_merged = rgb_img.merge_alpha(alpha_img)
    print(f"✓ Merged alpha channel back")
    
    # Convert alpha to color
    img_alpha_colored = img.alpha_to_color((128, 128, 128))
    print(f"✓ Converted alpha channel to gray color")
    ...
    print(f"✓ Analyzed color distribution")
    ...
    print(f"✓ Combined masks with multiply operation")
