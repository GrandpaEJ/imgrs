...
pub fn draw_star(
    ...
    for i in 0..(points * 2) {
        ...
        let radius = if i % 2 == 0 { outer_radius as f32 } else { inner_radius as f32 };
...
pub fn draw_ellipse(
    ...
    match &mut result {
        DynamicImage::ImageRgb8(rgb_img) => {
            ...
            for y in (center_y - radius_y as i32).max(0)..(center_y + radius_y as i32 + 1).min(img_height as i32) {
                for x in (center_x - radius_x as i32).max(0)..(center_x + radius_x as i32 + 1).min(img_width as i32) {
