                ...
                for y in 0..height {
                    for x in 0..width {
                        let distance = ((x as f32 - center_x as f32).powi(2) + 
                                      (y as f32 - center_y as f32).powi(2)).sqrt();
    ...
    pub fn create_color_mask_impl(&mut self, target_color: (u8, u8, u8), tolerance: u8, feather: u32) -> Result<DynamicImage, ImgrsError> {
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                let alpha = if distance <= tolerance as f32 {
                    255
                } else {
                    0
                };
                ...
                "multiply" => {
                    for y in 0..height {
                        for x in 0..width {
                            ...
                            let mask_pixel = mask_rgba.get_pixel(x, y);
                            let combined_alpha = ((result_pixel[3] as f32 * mask_pixel[3] as f32) / 255.0) as u8;
                ...
                "add" => {
                    for y in 0..height {
                        for x in 0..width {
                            let result_pixel = result.get_pixel(x, y);
                            let mask_pixel = mask_rgba.get_pixel(x, y);
                            let combined_alpha = (result_pixel[3] + mask_pixel[3]).min(255);
                            result.put_pixel(x, y, Rgba([255, 255, 255, combined_alpha]));
                        }
                    }
                },
                "subtract" => {
                    for y in 0..height {
                        for x in 0..width {
                            let result_pixel = result.get_pixel(x, y);
                            let mask_pixel = mask_rgba.get_pixel(x, y);
                            let combined_alpha = (result_pixel[3] as i16 - mask_pixel[3] as i16).max(0) as u8;
                            result.put_pixel(x, y, Rgba([255, 255, 255, combined_alpha]));
                        }
                    }
                },
                "overlay" => {
                    for y in 0..height {
                        for x in 0..width {
                            ...
                            result.put_pixel(x, y, Rgba([255, 255, 255, (combined_alpha * 255.0) as u8]));
                        }
                    }
                },
                _ => {
                    return Err(ImgrsError::InvalidOperation(format!("Unknown mask operation: {}", operation)));
                }
            }
        }
    ...
    pub fn color_quantize_impl(&mut self, levels: u8) -> Result<Self, ImgrsError> {
        ...
        for y in 0..rgba_image.height() {
            for x in 0..rgba_image.width() {
                ...
                result.put_pixel(x, y, Rgba([quantized_r, quantized_g, quantized_b, pixel[3]]));
            }
        }
        ...
        for y in 0..rgba_image.height() {
            for x in 0..rgba_image.width() {
                ...
                if distance <= tolerance as f32 {
                    ...
                    let final_g = (pixel[1] as f32 * (1.0 - desaturate_factor) + gray as f32 * desaturate_factor) as u8;
                    let final_b = (pixel[2] as f32 * (1.0 - desaturate_factor) + gray as f32 * desaturate_factor) as u8;
    ...
    pub fn color_match_impl(&mut self, reference_image: DynamicImage, strength: f32) -> Result<Self, ImgrsError> {
        ...
        for y in 0..height {
            for x in 0..width {
                ...
                let matched_r = (pixel[0] as f32 * (1.0 - strength) + ref_pixel[0] as f32 * strength) as u8;
                let matched_g = (pixel[1] as f32 * (1.0 - strength) + ref_pixel[1] as f32 * strength) as u8;
                let matched_b = (pixel[2] as f32 * (1.0 - strength) + ref_pixel[2] as f32 * strength) as u8;
