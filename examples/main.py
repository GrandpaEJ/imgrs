...
def select_anime(id=None, name=None, category=None):
    ...
    if category is not None:
        candidates = [a for a in candidates if a['category'].lower() == category.lower()]
...
async def generate_image(anime, custom_name=None, slogan=None):
    ...
    # Process in thread
    result = await asyncio.to_thread(process_image, anime, image_data, custom_name, slogan)
    return result
...
def process_image(anime, image_data, custom_name=None, slogan=None):
    ...
    try:
        ...
        layer4 = Image.new(size=(1800, 450), color=(255, 255, 255), mode='RGB')\
        ...
        newimg = Image.new('RGBA', (1500, 500), (0, 0, 0, 0))\
            .add_text_styled(
                text=name_text,
                position=(100, 0),
                size=300,
        ...
        # Add standard text
        img = img.add_text(text=name_text.upper(), position=(400, 380), size=67, font_path="fonts/GMVDINPro-CondMedium.ttf")
        img = img.add_text(text=slogan_text, position=(400, 440), font_path="fonts/arial/ARIAL.TTF")
