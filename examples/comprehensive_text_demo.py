...
def create_comprehensive_demo():
    ...
    image = image.draw_rectangle(
        btn1_x, btn1_y, btn1_width, btn1_height,
        (70, 130, 180, 255)
    )
    ...
    image = image.draw_rectangle(
        btn2_x, btn2_y, btn1_width, btn1_height,
        (108, 117, 125, 255)
    )
    ...
    image = image.draw_rectangle(
        btn3_x, btn3_y, btn1_width, btn1_height,
        (220, 53, 69, 255)
    )
    ...
    for size, width_px, height_px, ascent, descent in measurements:
        measurement_text += f"Size {size}: {width_px}x{height_px}px (A:{ascent}, D:{descent})\n"
...
def create_before_after_comparison():
    ...
    image = draw_text_box_compat(
        image, before_text.strip(),
        50, 80, center_x - 100, 400,
        14, (80, 0, 0, 255), (255, 220, 220, 255),
        "left", "top", 1.2, False
    )
...
def draw_text_box_compat(image, text, x, y, width, height, size, color, bg_color, align, valign, line_spacing, overflow):
...
def main():
    ...
    try:
        ...
        output_path1 = os.path.join(os.path.dirname(__file__), 'output', 'comprehensive_text_demo.png')
        main_image.save(output_path1)
        print(f"✅ Main demo saved to: {output_path1}")
        
        # Create before/after comparison
        comparison_image = create_before_after_comparison()
        output_path2 = os.path.join(os.path.dirname(__file__), 'output', 'text_improvements_comparison.png')
